...
image = modal.Image.debian_slim(
    python_version="3.12").pip_install_from_requirements("requirements.txt")
...
class MusicFlowMatching(nn.Module):
    def __init__(self, hidden_dim=256, sequence_length=150, num_layers=4):
        ...
        self.bert = BertModel.from_pretrained(
            'prajjwal1/bert-tiny', cache_dir="/my_vol/bert-tiny")
    ...
    def pad_or_truncate_tokens(self, tokens):
        ...
        if current_length > self.sequence_length:
            ...
            padding = torch.zeros(self.sequence_length -
                                  current_length, dtype=tokens.dtype)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % 10 == 0:
            grad_norm = torch.nn.utils.clip_grad_norm_(
                self.parameters(), self.trainer.gradient_clip_val)
